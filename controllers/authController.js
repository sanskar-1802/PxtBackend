...
//     const found = await RefreshToken.findOne({ user: payload.id, token });
//     if (!found) return res.status(401).json({ message: 'Invalid refresh' });
...
//   // In production, you'd store hash + expiry; for sample, we send token directly
//   const resetLink = "http://your-frontend/reset-password?token=" + resetToken + "&id=" + user._id;
//   await sendMail({ to: user.email, subject: 'Password Reset', text: 'Reset: ' + resetLink });
//   res.json({ message: 'Email sent (check logs in dev)' });
// };
